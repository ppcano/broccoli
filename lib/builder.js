...
Builder.prototype.build = function (willReadStringTree) {
  ...
  return RSVP.resolve()
    ...
    .then(function (node) {
      return { directory: node.directory, graph: node, nodes: self.currentNodes, totalTime: node.totalTime }
    })
    .catch(function (err) {
...
Builder.prototype._read = function (tree, willReadStringTree) {
    ...
    node = getNodeByTree(this.allNodes, tree);
    if (node) {
      ...
      if ( hasUnchangedDeps(node) ) {
        ...
        //TODO: add current timing to the node build
        // remove from allNodes, because it will be added after
...
/*
 * If a node has dependencies, it delegates its result to its dependent nodes
 * Otherwise the result is the return value of the`isUnchanged` method or false
 * when `isUnchanged` method is not implemented
 */
function hasUnchangedDeps (node) {

  // TODO: we could cache the result of a `hasUnchangedDeps` processed node to improve the exec time of other nodes which depends on it
  // I think, it is not urgent because multiple nodes does not usually share the same node dependencies
  
  var tree = node.tree,
      length = node.deps.length,
      result = false;
  ...
  if (typeof tree !== 'string') {
    ...
    if ( length > 0 ) {
      result = node.deps.every(function(depNode) {
        return hasUnchangedDeps(depNode);
      });

    } else if (tree.isUnchanged && (typeof tree.isUnchanged) === 'function' ){

      result = tree.isUnchanged();
